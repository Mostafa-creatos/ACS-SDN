...
// node_modules/cytoscape-dagre/dist/cytoscape-dagre.mjs
...
function norm(v2) {
  var len = Math.hypot(v2.x, v2.y) || 1;
  return {
    x: v2.x / len,
    y: v2.y / len,
    len
...
function buildEdgeFrame(src, tgt) {
  ...
  var _norm = norm(d), x2 = _norm.x, y = _norm.y, len = _norm.len;
  var dir = {
    x: x2,
    y
  };
...
function noZero(x2) {
  if (Math.abs(x2) < EPSILON) {
    return x2 < 0 ? -EPSILON : EPSILON;
  }
  return x2;
}
function toEdgeCoordinates(P2, frame) {
  var vector = subtract(P2, frame.src);
...
function normalizeWeight(coords) {
  var min = Infinity;
  var max = -Infinity;
  var _iterator = _createForOfIteratorHelper(coords), _step;
  try {
  ...
  var range = max - min || 1;
  return coords.map(function(p3) {
    return {
      distance: p3.distance,
      weight: (p3.weight - min) / range
    };
  });
}
function dagreEdgeToCytoscapeEdge(dEdge, cEdge) {
  ...
  var coords = normalizeWeight(dEdge.points.map(function(p2) {
    return toEdgeCoordinates(p2, frame);
  }));
  var controlPointWeights2 = coords.slice(1, -1).map(function(c) {
    return c.weight;
  });
  var controlPointDistances2 = coords.slice(1, -1).map(function(c) {
    return c.distance;
  });
  var result = {
    controlPointWeights: controlPointWeights2,
    controlPointDistances: controlPointDistances2
  };
  return result;
}
DagreLayout.prototype.run = function() {
  ...
  if (options.useDagreEdgeControlPoints) {
    ...
    g.edges().forEach(function(id2) {
      var cyEdge = cy.getElementById(id2.name);
      var dEdge = g.edge(id2);
...
var register = function register2(cytoscape) {
  if (!cytoscape) {
    return;
  }
...
cytoscape-dagre/dist/cytoscape-dagre.mjs:
  (*!
