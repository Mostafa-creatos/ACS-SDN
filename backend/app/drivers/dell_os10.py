...
def _push_via_collector(collector: DellOS10Collector, transport: str, config_payload: str) -> dict:
    ...
    try:
        ...
        # Check current prompt mode safely without triggering EXEC-mode 'end' error
        ...
        for line in config_payload.strip().splitlines():
            ...
            # Filter spurious "% Error: Unrecognized command." generated by 'end' at root EXEC prompt
            out_clean = out.replace("end\n% Error: Unrecognized command.", "").replace("% Error: Unrecognized command.", "")
            if any(hint in out_clean for hint in OS10_ERROR_HINTS):
                if "% Error" in out_clean and "Unrecognized command" in out:
                    pass
                else:
                    collector._send_command("end")
        ...
        if any(hint in save_out for hint in OS10_ERROR_HINTS if hint != "ERROR:" and "Unrecognized command" not in save_out):
            # Fallback to copy running-config startup-configuration
