...
def _push_via_collector(collector: DellOS10Collector, transport: str, config_payload: str) -> dict:
    ...
    import time
    try:
        # Thoroughly drain socket input buffer of any leftover show command output or banners
        collector._send("\n\n".encode("utf-8"))
        time.sleep(0.3)
        ...
        if any(hint in save_out for hint in OS10_ERROR_HINTS if hint != "ERROR:" and "Unrecognized command" not in save_out):
        ...
        if real_errors and "% Error" in save_out and "Unrecognized command" not in save_out:
