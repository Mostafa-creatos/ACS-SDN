...
class TestDeviceInventory(unittest.TestCase):
    ...
    def test_create_and_delete_switch(self):
        ...
        response = self.client.get("/api/v5/visibility/inventory", headers=self.headers_admin)
        sw = next(s for s in response.json() if s["switch_id"] == switch_id)
        ...
        # Verify it is gone
        response = self.client.get("/api/v5/visibility/inventory", headers=self.headers_admin)
        found = any(sw["switch_id"] == switch_id for sw in response.json())
    ...
    def test_collect_snapshot(self):
        ...
        response = self.client.get("/api/v5/visibility/inventory", headers=self.headers_admin)
        sw_details = next(s for s in response.json() if s["switch_id"] == str(switch_id))
