...
if ($SYNC_BACKEND) {
    ...
    $runCmds += " docker run -d --name sdn_controller_app --network sdn-controller_default --network-alias app -p 8000:8000 -e DATABASE_URL=postgresql://sdn_admin:sdn_secure_password@db:5432/sdn_controller -e REDIS_URL=redis://redis-master:6379/0 -e \`"REDIS_SENTINEL_HOSTS=redis-sentinel-1:26379;redis-sentinel-2:26379;redis-sentinel-3:26379\`" -e REDIS_SENTINEL_MASTER=mymaster -e SEED_ADMIN_PASSWORD=admin_password_123! -e SEED_OPERATOR_PASSWORD=operator_password_123! -e SEED_AUDITOR_PASSWORD=auditor_password_123! -e SEED_ON_STARTUP=false -e GNMI_DEFAULT_PASSWORD=NokiaSrl1! --restart unless-stopped sdn-controller_app:latest;"
    ...
    $runCmds += " docker run -d --name sdn_flower --network sdn-controller_default --network-alias flower -p 5555:5555 -e REDIS_URL=redis://redis-master:6379/0 -e \`"REDIS_SENTINEL_HOSTS=redis-sentinel-1:26379;redis-sentinel-2:26379;redis-sentinel-3:26379\`" -e REDIS_SENTINEL_MASTER=mymaster --restart unless-stopped sdn-controller_flower:latest celery -A app.workers.celery_app flower --port=5555 --basic_auth=admin:admin;"
}
if ($SYNC_FRONTEND) {
